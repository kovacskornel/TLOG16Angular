"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var day_1 = require(".././day");
var router_1 = require("@angular/router");
var app_component_1 = require("../../app.component");
var pager_service_1 = require(".././pager.service");
var WorkdayComponent = (function () {
    function WorkdayComponent(router, pagerService) {
        this.router = router;
        this.pagerService = pagerService;
        this.app = app_component_1.AppComponent;
    }
    WorkdayComponent.prototype.navigateTaskList = function () {
        this.pagerService.selectedDay = this.day;
        this.router.navigate(['/task-list']);
    };
    __decorate([
        core_1.Input(), 
        __metadata('design:type', day_1.Day)
    ], WorkdayComponent.prototype, "day", void 0);
    WorkdayComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'workday',
            templateUrl: 'workday.component.html',
            styleUrls: ['../../../styles.css']
        }), 
        __metadata('design:paramtypes', [router_1.Router, pager_service_1.PagerService])
    ], WorkdayComponent);
    return WorkdayComponent;
}());
exports.WorkdayComponent = WorkdayComponent;
//# sourceMappingURL=workday.component.js.map